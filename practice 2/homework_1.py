...
matrix = np.load("resources/matrix_43.npy")
...
with open("results/res_matrix_43.json", "w") as result:
    result.write(json.dumps(matrix_stat))

res_matrix = np.ndarray((size, size), dtype=float)

for i in range(0, size):
    for j in range(0, size):
        res_matrix[i][j] = matrix[i][j] / matrix_stat["sum"]

np.save("results/res_matrix_43", res_matrix)